...
It's rather simple, first [**read the docs**](https://docs.python.org/3/library/datetime.html#datetime.timedelta.seconds) and secondly, use `.total_seconds()`. Though, you have to know, that it returns float instead of int, so either use `math.ceil` or `math.floor` to get right value, depending on your usecase.
